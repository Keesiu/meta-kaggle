...
if __name__ == '__main__':
    ...
    for n,m,l in params:
        ...
        for ifold,(itr,icv) in enumerate(cross_validation.KFold(len(y),kfold,indices=False)):
            ...
            for irf in range(len(rfs.rfs)):
                ...
                print("%i,%i %.5f %.5f %.5f %.5f" % (
                    ...
                    np.mean(pG[ipt2&itr]==y[ipt2&itr]),np.mean(vmask[ipt2&itr]&(pG[ipt2&itr]==y[ipt2&itr]))), end=' ')
                if verbose_selection:
                    print(" ".join(["%.5f" %pts for pts in ptscore]), end=' ')
                    print(" ".join(["%.5f" %pts for pts in tptscore]), end=' ')
        ...
        print("%i %i %i\t %.2f %.2f %.4f %.4f %.2f - %.2fm" % (
            ...
            (time()-t)/60), end=' ')                                 # k-fold time
        print(" ".join(["%.5f" %pts for pts in np.mean(ptscores[(m,l)],axis=0)]), end=' ')
    ...
    if submit:
        ...
        # TEST SET PREDICTION
        print("now predicting on test set...", end=' ')
