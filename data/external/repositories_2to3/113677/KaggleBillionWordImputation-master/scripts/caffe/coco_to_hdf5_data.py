...
class CocoSequenceGenerator(SequenceGenerator):
  def __init__(self, coco, batch_num_streams, vocab=None,
    ...
    if split_ids is None:
      split_ids = list(coco.images.keys())
    ...
    num_no_sentences = 0
    for image_filename, metadata in known_images.items():
  ...
  def init_vocabulary(self, image_annotations, min_count=5):
    ...
    words_by_count = sorted(list(words_to_count.keys()), key=lambda w: (-words_to_count[w], w))
