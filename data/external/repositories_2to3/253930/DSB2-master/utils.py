...
#                    assert len(set(v)) == len(set(times))
                ...
                if times is None:
                    times = set(sum(list(scan_times.values()),[]))
                else:
                    times |= set(sum(list(scan_times.values()),[]))
            ...
            if maxscan is not None and maxscan > 2:
                if self.verbose:
                    print(self.name,"expanding slice %d"%s,maxscan,list(scan_times.keys()))
                for scan in list(scan_times.keys()):
                    if scan == maxscan:
                        continue
                    assert scan != 0
                    s1 = s + scan/100.
...
#                print 'angle change','%f %f'%(self.PatientAngle,angle)
    ...
    def _read_all_dicom_images(self):
        ...
        # immutable fields
        list(map(self.sex, itertools.chain(*dicoms)))
        list(map(self.angle, itertools.chain(*dicoms)))
        list(map(self.age, itertools.chain(*dicoms)))
        list(map(self.patient_position, itertools.chain(*dicoms)))
        ...
        idxs = [s for s in range(S) if tuple(np.around(np.cross(dicoms[s][0].ImageOrientationPatient[:3],
        ...
        area_scales = [[x[1] for x in slice] for slice in images]
        ...
        images = [[x[0] for x in slice] for slice in images]
        ...
        n = len(idxs)
        idxs = [s for s in idxs if images[s][0].shape == self.shape]
        if len(idxs) < n:
            if self.verbose:
        ...
        for dd in idxs:
            ...
            for tt,t in enumerate(d):
                if t is None:
                    continue
                x1,y1 = list(map(float,t.PixelSpacing))
...
#                 x1 = np.around(x1,2)
        ...
        self.xxs = [x for x in xxs if np.around(x[0],4)!=np.around(self.area,4)]
        bad_slices = set([x[1] for x in self.xxs])
        ...
        if len(dicoms) > 1:
            ...
            maxh = max(h.values())
            self.dist = min([k for k,v in list(h.items()) if v==maxh])
            ...
            maxseq = max(list(map(len,sequences)))  # length of each sequence
            maxseq = [x for x in sequences if len(x)==maxseq][0]
