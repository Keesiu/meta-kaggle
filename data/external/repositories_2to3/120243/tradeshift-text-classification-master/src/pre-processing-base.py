...
def pre_processing_base(data_dir, data_base_dir):
    ...
    for name in train.columns :    
        if name.startswith('x') :
            column_type, _ = max(list(Counter([str(type(x)) for x in train[name]]).items()), key = lambda x: x[1])
        
        # LOL expression
            if column_type == str(str) :
                train[name] = list(map(str, train[name]))
    ...
    X_sparse = vec.fit_transform(list(train[names_categorical].T.to_dict().values()))
    X_test_sparse = vec.transform(list(test[names_categorical].T.to_dict().values()))
