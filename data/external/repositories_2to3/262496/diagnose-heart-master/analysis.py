...
def take_best(cfiles,method=2, filter_ll=None):
    ...
    if filter_ll is not None:
        mean, std = ll_means_stds([a_[:,1:] for sublist in [list(adic.values()) for adic in cfiles]
            for a_ in sublist])
    for c in css:
...
def take_best_contour(areafiles,contfiles,method=1, filter_ll=None):
    ...
    conts = {};
    if filter_ll is not None:
        mean, std = ll_means_stds([a_[:,1:] for sublist in [list(adic.values()) for adic in areafiles]
            for a_ in sublist])
    for c in css:
...
def clean_data(data,case,cleaner=[]):
    ...
    if 0 in cleaner:#smooth single wrong reads
        for i in reversed(list(range(1,min(L//2,3)))):
...
def get_preliminary_volume(areas_all, cleaner=[]):
    result = {};
    for case,areas in areas_all.items():
...
def fill_default(preds, default):
    final = {};
    for case,value in default.items():
        pred = preds.get(case);
        if pred is None:
            final[case] = value;
