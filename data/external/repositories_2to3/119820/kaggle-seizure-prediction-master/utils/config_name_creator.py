...
def create_cnn_model_name(settings_dict):
    config_name = ''
    for key, value in list(settings_dict['preprocessor'].items()):
        config_name += key[:5] + str(value)
    for key, value in list(settings_dict['model'].items()):
        config_name += key[:5] + str(value)
    for key, value in list(settings_dict['validation'].items()):
        config_name += key[:5] + str(value)
