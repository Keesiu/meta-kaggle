import sys, time, urllib.request, urllib.parse, urllib.error, code
from chess import *
...
class DSJURLopener(urllib.request.FancyURLopener):
    version = "dsjoerg"

urllib.request._urlopener = DSJURLopener()
...
def do_it_backwards(engine, game=None, debug=False, movenum=None):
    ...
    outstruct = {}
    outstruct['event'] = game.headers['Event']
    outstruct['position_scores'] = [list() for _ in range(len(analysis_times))]
    outstruct['best_moves'] = [list() for _ in range(len(analysis_times))]
    outstruct['move_features'] = [list() for _ in range(len(analysis_times))]
    outstruct['best_move_features'] = [list() for _ in range(len(analysis_times))]
    outstruct['depth_stats'] = [list() for _ in range(len(analysis_times))]
    outstruct['material_stats'] = [list() for _ in range(len(analysis_times))]
    outstruct['gb'] = [list() for _ in range(len(analysis_times))]
    outstruct['gb12'] = [list() for _ in range(len(analysis_times))]
    outstruct['gbN'] = [list([0] * (NUM_GB_DEPTHS+1)) for _ in range(len(analysis_times))]

    was_bestmove = [list() for _ in range(len(analysis_times))]
