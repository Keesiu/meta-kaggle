...
ALL_TRAIN_PATIENT_IDS = list(range(TRAIN_PATIENT_IDS[0], TRAIN_PATIENT_IDS[1] + 1))
...
def _construct_id_to_index_map(patient_folders):
    ...
    for set in patient_folders:
        for index in range(len(patient_folders[set])):
...
test_patients_indices =  list(range(TEST_PATIENT_IDS[0], TEST_PATIENT_IDS[1] + 1))
# Aggregate in a dict
patient_folders = {
...
def filter_patient_folders():
    ...
    for set, key in patient_folders.items():
...
_hough_rois = utils.merge_dicts(list(map(_load_file, _HOUGH_ROI_PATHS)))
...
def get_patient_data(indices, wanted_input_tags, wanted_output_tags,
    ...
    if hasattr(_config(), 'check_inputs') and _config().check_inputs:
        for key, value in itertools.chain(iter(result["input"].items()), iter(result["output"].items())):
...
def generate_validation_batch(required_input_keys, required_output_keys, set="validation"):
    ...
    for n in range(num_chunks):
        ...
        if "sunny" in input_keys_to_do or "segmentation" in output_keys_to_do:
            ...
            indices = list(range(n*sunny_chunk_size, (n+1)*sunny_chunk_size))
        ...
        indices = list(range(n*regular_chunk_size, (n+1)*regular_chunk_size))
...
def generate_test_batch(required_input_keys, required_output_keys, augmentation=False, set=None):
    ...
    for set in sets:
        ...
        indices_for_this_set = range(regular_length)
        ...
        for n in range(num_chunks):
            ...
            test_sample_numbers = list(range(n*regular_chunk_size, (n+1)*regular_chunk_size))
