...
##6180/7954 = 0.7770 accuracy +0.007
    if sum(d.values()) > 1:
        offset = random.randint(0, int(sum(d.values()))-1)
        for k, v in d.items():
            if offset < v:
                return k
            offset -= value
    else:
        return max(iter(d.keys()), key=lambda k: d[k])
...
def get_ingredient_count_class(ing_count, cuisine_type_bag):
    ...
    for each in ing_count:
        for cuisine,ingredients in list(cuisine_type_bag.items()):
            #print ingredients
...
def fullseq_features(bow, stat_cuisine):
    ...
    for cuisine in stat_cuisine:
        ...
        for ingredient, value in bow.items() :
    ...
    for pass_iteration in range(numpasses):
        ...
        for bow, goldlabel in select_sam:
            ...
            if predlabel != goldlabel:
                ...
                for feat_name, feat_value in goldfeats.items():
                ...
                for feat_name, feat_value in predfeats.items():
                    weights[feat_name] -= stepsize * feat_value
                    weightSums[feat_name] -= (t-1) * stepsize * feat_value
...
def plot_accuracy_vs_iteration(train_acc, test_acc, avg_test_acc, naive_bayes_acc = 0.73):
    ...
    plt.plot(list(range(len(avg_test_acc))), avg_test_acc)
    plt.plot(list(range(len(avg_test_acc))), [naive_bayes_acc] * len(avg_test_acc))
...
if __name__=='__main__':
    ...
    for key, value in list(ing_count_adj.items()):
