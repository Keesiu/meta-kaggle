# this is the benchmark from https://www.kaggle.com/anlthms/grasp-and-lift-eeg-detection/convnet-0-89/run/45011
import  logging
import   os
import  numpy as np
import pandas as pd
...
class GalData(Dataset):
    ...
    def readfiles(self, dirname, serlist):
        ...
        """
        basepath = os.path.join(os.pardir, '../data', dirname)
        ...
        for series in serlist:
            ...
            if dirname == 'train':
                ...
                filename = 'subj{}_series{}_events.csv'.format(self.subj, series)
                path = os.path.join(basepath, filename)
    ...
    def load(self, **kwargs):
        ...
        if validate:
            ...
            train, trainlabs, _ = self.readfiles('train', list(range(1, 9)))
