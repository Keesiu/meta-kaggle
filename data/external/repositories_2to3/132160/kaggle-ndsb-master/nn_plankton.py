...
class AdversarialRegObjective(object):
    ...
    def get_loss(self, target=None, *args, **kwargs):
        ...
        input_grad_map = { layer: T.grad(loss, input_var) for layer, input_var in self.input_map.items() }
        perturbed_input_map = { layer: input_var + self.epsilon * T.sgn(input_grad_map[layer]) for layer, input_var in self.input_map.items() }
