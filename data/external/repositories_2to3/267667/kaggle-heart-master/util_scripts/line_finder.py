...
def find_line(coordinates, orientations, labels=None):
    ...
    """
    if labels is None:
        labels = list(range(len(coordinates)))

    p = list(zip(*(coordinates, orientations, list(range(len(coordinates))))))
    ...
    groups = []
    for key in list(groups_by_orientation.keys()):
        ...
        for line in lines:
            if len(line) > 2:
                ...
                '''
                groups.append(list(zip(*param))[1])
...
if __name__ == "__main__":
    ...
    for patient_folder in patient_folder_list:
        ...
        labels, positions, orientations = list(map(list, list(zip(*sorted(zip(labels, positions, orientations))))))
