...
class draw_repeated_trip:
    ...
    def draw(self):
        ...
        indx = np.arange(1, 201)
        rs = np.vstack((indx, out)).T
        np.savetxt("rep_trips/%s.txt" % driver, rs, fmt='%d')
        # plt.show()


def paral_stuff(path, driver):
...
if __name__ == "__main__":
    ...
    drivers = [idd for idd in folders if idd[0] != "." and 0 < int(idd)]

    n_proc = cpu_count()

    Parallel(n_jobs=n_proc)(delayed(paral_stuff)(path, d) for d in drivers)
