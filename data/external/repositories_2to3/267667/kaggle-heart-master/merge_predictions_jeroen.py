...
def _construct_all_patient_ids():
    ...
    for arr in list(all_patient_ids.values()):
        arr.sort()
...
def _add_tta_score_to_metadata(metadata, tta_score, best_method):
    ...
    metadata['best_method'] = best_method.__name__
...
def _compute_best_tta(metadata, pats_predicted, best_method=None):
    ...
    if not best_method:
        ...
        for averaging_method in AVERAGING_METHODS:
            ...
            print("    - %s: %.5f" % (averaging_method.__name__, err))
            if err < best_score:
                best_score, best_method = err, averaging_method
    else:
        print("    Using predefined averaging")
        for averaging_method in AVERAGING_METHODS:
            if best_method == averaging_method.__name__:
    ...
    print("    Choosing %s (%.5f)" % (best_method.__name__, tta_score))
...
def _find_weights(w_init, preds_matrix, targets_matrix, mask_matrix, eps=0.0, use_reg=True, slice_models_idx=None):
    ...
    for iteration in range(nr_epochs):
...
def _create_ensembles(metadatas, outliers=None, slice_ensemble_results=None, weights=None):
    ...
    sorted_models, sorted_w_sys, sorted_w_dia = list(zip(*sorted(zip(metadatas, w_sys, w_dia), key=sort_key)))
    ...
    if DO_XVAL:
        ...
        for val_pid in range(nr_val_patients):
            ...
            # Select models
            sorted_models_i, sorted_w_sys_i, sorted_w_dia_i = list(zip(*sorted(zip(metadatas, w_sys_i, w_dia_i), key=sort_key)))
