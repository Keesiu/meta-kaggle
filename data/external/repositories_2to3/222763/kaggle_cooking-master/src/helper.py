...
class RecipeModel():
	...
	def set_weights(self,pred_a,pred_b,target):
		...
		for w in zip(list(range(1,100,1)),list(range(99,0,-1))):
	...
	def predict_kaggle(self,X,prob=False):
		...
		for v in list(self.text_models.keys()):
...
def trainText(model_a,modelGrid_a,model_b,modelGrid_b,train,target,cv,refit=True,n_jobs=5):
	...
	best_score = 0
	for w in zip(list(range(2,100,2)),list(range(98,0,-2))):
...
def trainFeatureModel(train,target,model,grid,cv,n_jobs=-1):
	...
	for g in ParameterGrid(grid):
		model.set_params(**g)
		if len([True for x in list(g.keys()) if x.find('nthread') != -1 or x.find('n_jobs') != -1 ]) > 0:
