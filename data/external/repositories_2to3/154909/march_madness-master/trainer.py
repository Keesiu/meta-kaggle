...
class Features:
    ...
    def features(self):
        if self.daynum is None:
            team_one_features = max([(k, v) for k, v in list(self.team_one.features.items()) if k[0] == self.season])[1]
            team_two_features = max([(k, v) for k, v in list(self.team_two.features.items()) if k[0] == self.season])[1]
...
class AllFeatures:
    ...
    def features_and_labels(self, season):
        ...
        if os.path.exists(feature_pickle) and os.path.exists(label_pickle):
            return pickle.load(open(feature_pickle)), pickle.load(open(label_pickle))
...
class TunedModel:
    ...
    def print_test(self, test):
        params = ", ".join(["{:s} = {:s}".format(str(key), str(value)) for key, value in list(test["params"].items())])
    ...
    @staticmethod
    def clean():
        list(map(os.remove, glob.glob(os.path.join(JSON_DIR, "*"))))
...
def season_models(season):
    ...
    def ensemble_builder(**params):
        filtered_models = {k: v for k, v in models.items() if "Ensemble" not in k}
...
def cross_validate(season):
    ...
    raw_test_x, test_y = list(map(numpy.array, AllFeatures().features_and_labels(season)))
    scaler = StandardScaler()
    ...
    best_model = min(iter(models.values()), key=lambda j: j.best_params()["loss"])
...
class EnsembleModel:
    def __init__(self, models, **params):
        ...
        self.model_funcs = [j.model for j in list(models.values())]
        self.params = params
