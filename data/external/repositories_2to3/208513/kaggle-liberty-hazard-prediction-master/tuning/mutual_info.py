...
def gini(solution, submission):
    df = list(zip(solution, submission, list(range(len(solution)))))
...
mi_sorted = sorted(list(mi_dict.items()), key=lambda x:x[1], reverse=True)

nmi_dict = {}
for i in range(train.shape[1]):
    nmi_dict[columns[i]] = nmi[i]
nmi_sorted = sorted(list(nmi_dict.items()), key=lambda x:x[1], reverse=True)

ami_dict = {}
for i in range(train.shape[1]):
    ami_dict[columns[i]] = ami[i]
ami_sorted = sorted(list(ami_dict.items()), key=lambda x:x[1], reverse=True)
