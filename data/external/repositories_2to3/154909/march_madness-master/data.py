                ...
                for key, value in row.items():
                    ...
                    while not has_type:
                        try:
                            ...
                            column_types[key] = testers[column_types[key]].__next__
        return list(column_types.items())
...
def human_readable():
    ...
    with open(os.path.join(DIR, 'out_data/predictions/2015_ensemble.csv')) as buff:
        reader = csv.DictReader(buff)
        for row in reader:
            _, team_one, team_two = list(map(int, row["id"].split("_")))
...
def check_first_round():
    ...
    for region, region_data in teams.items():
        for seed, seed_data in region_data.items():
