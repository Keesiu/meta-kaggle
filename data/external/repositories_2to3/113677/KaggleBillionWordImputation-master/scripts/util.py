...
def ngram_frequencies2(istream, n=1):
    ...
    for i, line in enumerate(istream):
        if i % 100000 == 0:
            print("Line %d (%d 1-grams, %d %d-grams)" \
                % (i, len(unigrams), len(counts), n), file=sys.stderr)
        words = tokenize_words(line)
        ids = words2ids(words, unigrams)
        for ngram in window(ids, n):
            counts[ngram] += 1
    id2word = {v: k for k, v in unigrams.items()}
    del unigrams
...
def prune_vocab(vocab, n):
    nwords = sum(v for v in vocab.values())
    nvocab = len(vocab)
    print("Input has nwords = %s, vocab size = %d" \
        % (nwords, nvocab), file=sys.stderr)
    vocab = [(v,k) for k,v in vocab.items()]
    vocab = list(reversed(sorted(vocab)))
    vocab = vocab[:n]
    vocab = {k: v for v, k in vocab}
    nremaining = sum(v for v in vocab.values())
    percent_kept = float(len(vocab)) / nvocab
    percent_mass = float(nremaining) / nwords
    print("Keeping %d words (%.2f%% of vocab, %.2f%% of mass)" \
        % (len(vocab), 100*percent_kept, 100*percent_mass), file=sys.stderr)
    return vocab
    
def score(golden, predicted):
    total_d = 0.0
    n = 0
    for ref, pred in zip(golden, predicted):
...
def estimate_probabilities(ngrams):
    ...
    ntotal = float(sum(ngrams.values()))
    print("%d total syntactic ngrams" % ntotal)
    p = {k: np.log10(v/ntotal) for k, v in ngrams.items()}
    print("Total probability = %f" % sum(10.**v for v in p.values()))
    return p
...
class Word2Vec(object):
    ...
    def load(cls, istream):
        ...
        nwords = int(header[0])
        d = int(header[1])
        print("Allocating %dx%d word vector matrix" \
            % (nwords, d), file=sys.stderr)
        words = []
        ...
        for i, line in enumerate(istream):
            ...
            word = entry[0]
            words.append(word)
            V[i] = list(map(float, entry[1:]))
            if i % 500000 == 0: print(i, file=sys.stderr)
        return cls(words, V)
...
class Prediction(object):
    ...
    def parse(cls, line):
        ...
        word = entry[0]
        # locations
        loc = list(map(int, entry[1:cls.keep_top_n]))
        # probabilities
        for i in range(cls.keep_top_n+1, len(entry)):
...
class TopK(object):
    ...
    def __iter__(self):
        return zip(self.things, self.values)