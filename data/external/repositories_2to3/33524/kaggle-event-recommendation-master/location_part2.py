...
for tz in list(timezones.keys()):
    ...
    countries = sorted(list(countries.items()), key=lambda x: -x[1])
...
for uid in list(user_data.keys()):
    ...
    if user['locale'] not in locales:
        continue
    locale_country = locales[user['locale']]
    if locale_country not in list(tz_data.keys()):
        continue
    sw = 0
...
for uid in user_ids:
    ...
    # filter
    friend_votes = list(friend_votes.values())
...
keys = list(friend_loc.keys())
for k in keys:
    if not friend_loc[k]:
        friend_loc.pop(k)

friend_locs_dict = {}
for fuid in friend_loc.keys():
    votes = {}
...
# get location from attendants
count = 0
for e, uids in update_based_on.items():
    count += 1
    if count % 1000 == 0:
        print(count)
    if not uids:
