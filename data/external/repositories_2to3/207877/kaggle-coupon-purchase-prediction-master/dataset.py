...
class Dataset:
    ...
    def __load_users(self):
        ...
        for i, user in user_df.iterrows():
            ...
            if i % 100 == 0:
                print("load users: %d/%d\r" % (i, len(user_df)), end=' ')
                sys.stdout.flush()
    ...
    def gen_train_data(self, num_nega=2, verbose=True):
        ...
        for user in self.users:
            ...
            for i in range(len(user_coupons)):
                ...
                rid = coupon_ids[i]
                nega_list = list(range(max(0, rid - self.COUPON_DISP_NEAR), rid))
            ...
            if verbose and c % 100 == 0:
                print(("train data .. %d/%d\r" % (c, len(self.users))), end=' ')
                sys.stdout.flush()
        if verbose:
            print("\n", end=' ')
    ...
    def each_valid(self, callback, verbose=False):
        for k, user in enumerate(self.users):
            ...
            feats = np.empty((len(self.valid_coupon_vec),
                             ...
                             dtype=np.float32)
            coupon_ids = []
            for i in range(len(self.valid_coupon_vec)):
                coupon_id = i
            ...
            if verbose and (k % 100 == 0):
                print(("each valid .. %d/%d\r" % (k, len(self.users))), end=' ')
                sys.stdout.flush()
        if verbose:
            print("\n", end=' ')
    ...
    def each_test(self, callback, verbose=True):
        for k, user in enumerate(self.users):
            ...
            feats = np.empty((len(self.test_coupon_vec),
                             ...
                             dtype=np.float32)
            coupon_ids = []
            for i in range(len(self.test_coupon_vec)):
            ...
            callback({"user_id": user_id, "coupon_ids": coupon_ids, "coupon_feats": feats})
            if verbose and k % 100 == 0:
                print(("each test .. %d/%d\r" % (k, len(self.users))), end=' ')
                sys.stdout.flush()
        if verbose:
            print("\n", end=' ')
