...
def loadAuthors(authorfile, printaffilwordfreq=False):
	...
	for i, line in verbose_iter(reader):
		line[1:] = [unidecode(str(cell, 'utf-8')) for cell in line[1:]]
		...
		ai = {k: strip_punc(v.lower().encode('ascii'), space_dashes=False) for k, v in ai.items()}
	...
	if printaffilwordfreq:
		print("-----")
	name_tfidf = computeTFIDFs(fullnames, None, min_df=2, ngram_range=(1,3), words_freq=printaffilwordfreq, token_pattern='(?u)\\b[a-zA-Z][a-zA-Z]+\\b')
	if printaffilwordfreq:
		return

	print_err("Calculating IDFs")
	iFfL_IDF = dict(list(zip(list(iFfL_cnt.keys()), np.log(float(len(authors)) / np.array(list(iFfL_cnt.values()))))))
	lastname_IDF = dict(list(zip(list(lastname_cnt.keys()), np.log(float(len(authors)) / np.array(list(lastname_cnt.values()))))))
