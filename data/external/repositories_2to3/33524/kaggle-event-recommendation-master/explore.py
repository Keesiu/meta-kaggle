...
for name, matrix in requests.items():
    results[name] = []
    for i in range(len(matrix[0])):
        filtered = np.array([x for x in matrix[:,i] if x is not None])
...
stats = []
for uid, rec_dict in train_dict.items():
    if len(rec_dict) == 1:
        continue
    for ts, recs in rec_dict.items():
        c = len(recs)
