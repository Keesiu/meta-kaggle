...
def main():
	...
	if len(unloaded) == 4:
		...
		feat_ind_remaining = list(range(len(feat_indices)))
...
# 		print_err("No tree importances")
	...
	pl.bar(list(range(len(indices))), importances[indices],
		   color="r", yerr=std[indices], align="center")
	pl.xticks(list(range(len(indices))), [feat_indices[v] for v in indices], rotation=45, horizontalalignment='right')
