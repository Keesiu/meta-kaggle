...
chunks_train_ids = list(range(num_chunks_train))
losses_train = [np.inf]
...
for e, (xs_chunk, y_chunk, chunk_shape) in zip(chunks_train_ids,
                                                create_train_gen()):
    ...
    if ((e + 1) % output_every) == 0:
        ...
        for i, (layer, norm) in enumerate(zip_longest(layers_out, norms,
                                                       fillvalue=0)):
