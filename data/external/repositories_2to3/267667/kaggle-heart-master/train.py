...
def train_model(expid):
    ...
    if "pretrained" in interface_layers:
        for config_name, layers_dict in interface_layers["pretrained"].items():
            ...
            pretrained_top_layer = lasagne.layers.MergeLayer(
                incomings = list(layers_dict.values())
            )
    ...
    print(string.ljust("#data:",10), end=' ')
    print("output shape:")
    ...
    xs_shared = {
        key: lasagne.utils.shared_empty(dim=len(l_in.output_shape), dtype='float32') for (key, l_in) in input_layers.items()
    }
    ...
    ys_shared = {
        key: lasagne.utils.shared_empty(dim=target_var.ndim, dtype='float32') for (key, target_var) in obj.target_vars.items()
    }
    ...
    givens = dict()
    for key in list(obj.target_vars.keys()):
        if key=="segmentation":
    ...
    iter_validate = theano.function([idx], [validation_train_loss, validation_kaggle_loss, validation_segmentation_loss] + [v for _, v in list(validation_other_losses.items())] + theano_printer.get_the_stuff_to_print(),
    ...
    if config().restart_from_save and os.path.isfile(metadata_path):
        ...
        chunks_train_idcs = list(range(start_chunk_idx, num_chunks_train))
        ...
        chunks_train_idcs = list(range(num_chunks_train))
        losses_train = []
        losses_eval_valid = []
        losses_eval_train = []
    ...
    create_train_gen = partial(config().create_train_gen,
                               ...
                               required_output_keys = list(ys_shared.keys())# + ["patients"],
                               )
    ...
    create_eval_train_gen = partial(config().create_eval_train_gen,
                                   required_input_keys = list(xs_shared.keys()),
                                   required_output_keys = list(ys_shared.keys())
                                   )
    ...
    for e, train_data in zip(chunks_train_idcs, buffering.buffered_gen_threaded(create_train_gen())):
        ...
        for key, rate in learning_rate_schedule.items():
            if epoch >= key:
                lr = np.float32(rate)
        ...
        if ((e + 1) % config().validate_every) == 0:
            ...
            for subset, create_gen, losses_validation, losses_kgl in zip(subsets, gens, losses_eval, losses_kaggle):
                ...
                vld_other_losses = {k:[] for k,_ in list(validation_other_losses.items())}
                ...
                for validation_data in buffering.buffered_gen_threaded(create_gen()):
                    ...
                    for b in range(num_batches_chunk_eval):
                ...
                for k, v in list(vld_other_losses.items()):
