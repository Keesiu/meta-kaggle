...
def save_hd5py(dataset_dict, destfile, indices_dict_or_numfolds):
    ...
    if isinstance(indices_dict, int):
        folds = indices_dict
        n = max(len(it) for it in list(dataset_dict.values()))
        fold_n = n // folds
    ...
    for name, dataset in dataset_dict.items():
        ...
        dat[...] = dataset
    split_dict = dict((k, dict((name, v) for name in dataset_dict.keys()))
...
# labels are zero or one, not 255
    for contour_img in [x for x in os.listdir(aug_contour_path) if 'jpg' in x]:
