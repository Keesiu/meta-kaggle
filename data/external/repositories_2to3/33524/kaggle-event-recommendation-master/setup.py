...
updated = 0
for uid, friends in friends_info.items():
    s = 0
    nr = 0
...
etzf = {}
for eid, tz_dict in etz.items():
    tzl = list(tz_dict.items())
    tzl.sort(key=lambda x: -x[1])
    etzf[eid] = tzl[0]
    
    
count = len(etzf)
for eid, tz in etzf.items():
    count -= 1
    if count % 10000 == 0:
        print(count)
