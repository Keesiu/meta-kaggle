...
# 		'titles_dup',
	...
	def __init__(self, authorprefeat, authorfilterfile='data/authors_with_papers.txt'):
		self.filter = set(map(int, open(authorfilterfile, 'rb')))
		self.unpickled = {k: pickle.load(open(v, 'rb')) for k, v in self.pickles_.items()}
		self.conn = getDB()
 		self.conn_pa = getDB('pa')

		self.author_info = authorprefeat
 		for field, filename in self.pa_files_.items():
	...
	def getAuthor(self, aID):
		...
		authorinfo = next(selectDB(self.conn, self.author_query_, [aID]))
		...
		for field, query in self.pa_queries_author_.items():
	...
	def getCoauthorsSim(self, tablename, a1, a2):
		common, len1, len2 = next(selectDB(self.conn_pa, self.coauthor_query_.replace("pa_coauthors", tablename), [a1, a2, a1, a2]))
		return self.sim(common, len1, len2)

	def getCoauthorsSimW(self, tablename, a1, a2):
		common, len1, len2 = next(selectDB(self.conn_pa, self.coauthor_query_w_.replace("pa_coauthors", tablename), [a1, a2, a1, a2, a1, a2]))
		return self.sim(common, len1, len2)
	...
	def getYearScore(self, a1, a2):
		k1 = list(self.pa_by_authors[a1]['years'].items())
		k2 = list(self.pa_by_authors[a2]['years'].items())
		if not k1 or not k2:
			return 0
		kk1, kk2 = [], []
		for k, v in k1:
...
#  			print a, b
 			...
 			f = {k: '{:g}'.format(v) for k, v in f.items()}
