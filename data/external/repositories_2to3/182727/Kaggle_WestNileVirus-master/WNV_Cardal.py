...
def add_trap_bias(train, test, bias_factor=50.0):
    ...
    train['TrapBias'] = np.array([traps_bias[trap] for trap in train['Trap']], dtype=np.float32)
    test ['TrapBias'] = np.array([traps_bias[trap] for trap in test ['Trap']], dtype=np.float32)
...
def calc_mult_counts_prob(test, mult_entries_func):
    ...
    '''
    sp_tr_date_test = list(zip(test['Species'],test['Trap'],test['Address'],test['Date']))
...
def count_nearby_mult(ds, dist_func, max_days=7, max_lat=0.1, max_lng=0.1, same_trap=False, same_species=False, use_self=True, ignore_count1=False):
    ...
    '''
    sp_tr_date = list(zip(ds['Species'],ds['Trap'],ds['Address'],ds['Date']))
...
def prp_outbreaks_daily_factors(outbreaks, years, pow=2):
    ...
    for year,month,day,factor in outbreaks:
        ...
        for month2 in range(month-1,month+2):
            if month2 not in outbreaks_daily_factors[year]: continue
            for day2 in list(outbreaks_daily_factors[year][month2].keys()):
    ...
    for year in list(outbreaks_daily_factors.keys()):
        for month in list(outbreaks_daily_factors[year].keys()):
            for day in list(outbreaks_daily_factors[year][month].keys()): 
