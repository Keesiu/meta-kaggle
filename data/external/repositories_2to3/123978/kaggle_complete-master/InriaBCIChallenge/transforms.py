...
class SubjectFeedbackInfo:
    ...
    def transform(self):
        subjectlist = [int(x[1:3]) for x in self.subjsess_list]
        feedbacksesslist = [int(x[8:10]) for x in self.subjsess_list]
        X = DataFrame()
        xsubj = []
        xsess = []
        xfeedbacknum = []
        ...
        for findex in range(len(self.flist)):
            ...
            del x_df
            fb_nums = list(range(len(fb_indices)))
...
class Downsampler(BaseTransformer):
    ...
    def averager(self, X):
        ...
        for ep in range(n_epochs):
            for feat in range(int(n_features / self.rate)):
