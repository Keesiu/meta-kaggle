def scrambled_halton_sequence_generator(dimension=1, permutation=None):
    ...
    bases = [next(primes) for _ in range(dimension)]
    if permutation is None:
        generators = [halton_sequence_generator(base) for base in bases]
    elif permutation == 'reverse':
        generators = [halton_sequence_generator(base, permutation=[0] + [base-i for i in range(1,base)]) for base in bases]
    ...
    while True:
        yield [next(gen) for gen in generators]
...
def halton_sequence_generator(base, permutation=None, rational=False):
    if permutation is None:
        permutation = list(range(base)) #no permutation
    if permutation == 'pseudo-random':
        import random

    denominator = base
    while True:
        for i in range(denominator/base,denominator):
            if permutation == 'pseudo-random':
                l = list(range(1,base))
                random.shuffle(l)
                permutation = [0] + l
...
def pseudo_random_sequence_generator(dimension=1):
    import random
    while True:
        yield [random.random() for _ in range(dimension)]
...
if __name__=="__main__":
    ...
    samples = np.array([next(generator) for _ in range(points)])
