...
def prepare_data(n_drivers, windows=[1], n_quantiles=9, part=1, size=None, n_jobs=1, seed=7):
    ...
    print("Processing data | window %s | quantiles %i | (%i jobs) | ETA:" % (windows,n_quantiles,n_jobs), end=' ')
    # drawing sample (if under sampling)
    sample = [ driverids[i] for i in random.sample(range(len(driverids)), min(n_drivers,len(driverids))) ]
    # estimated time print iteration
    eta_iteration = (np.array([2,3,4,5,10,50,100])*n_jobs).tolist() + (np.array(list(range(200,3000,100))*n_jobs).tolist())
    ...
    if n_jobs > 1:
        ...
        for i,df in enumerate(pool.imap(pool_driver_trips_handler, [(trip,windows,n_quantiles,part,size) for trip in sample])):
            ...
            if i in eta_iteration[4:]:
                print("%.2fm (%.2fm)," % (((time()-t) / i * (len(driverids)-i+1) / 60.),(time()-t)/60.), end=' ')
                sys.stdout.flush()
        # terminate pool
        pool.terminate()
    else:
        dfs = [driver_trips(x,windows,n_quantiles,part) for x in sample]
