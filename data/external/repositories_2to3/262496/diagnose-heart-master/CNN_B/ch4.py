...
from . import dsb_utils as du, config as c, utils as u, models as m
...
def evaluate():
    ...
    for s in range(*c.ch4_eval_cases):
        if s % 10 == 0:
        ...
        if dset.ch4_images is not None:
            ...
            if sum(ch4counts) > 0:
                ...
                for t in range(dset.ch4seg.shape[0]):
                    diams = np.array([np.count_nonzero(dset.ch4seg[t,:,i])
                        for i in range(dset.ch4seg[t].shape[0])])
