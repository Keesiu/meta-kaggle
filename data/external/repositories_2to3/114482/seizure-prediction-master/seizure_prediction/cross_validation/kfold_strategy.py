    ...
    def get_sequence_ranges(self, meta, fold_number, interictal=False, shuffle=True):
        ...
        if interictal:
            ...
            if num_seqs == 3:
                ...
                first_pass = [list(range(i, i + train_size)) for i in range(0, num_seqs, train_size) if (i + train_size) <= num_seqs]
                remainder = num_seqs % train_size
                if remainder == 0:
                    gap = []
                else:
                    seq = list(range(num_seqs - remainder, num_seqs))
                    ...
                    if len(gap_fillers) < train_size:
                        ...
                        gap_fillers = [x for x in gap_fillers if x < num_seqs]
                    gap = [gap_fillers + seq]
                second_pass = [list(range(i, i + train_size**2, train_size)) for i in range(num_seqs)]
                second_pass = [x for x in second_pass if len(x) == train_size and x < num_seqs]
                third_pass = [list(range(i, i + train_size)) for i in range(1, num_seqs, train_size) if (i + train_size) <= num_seqs]
