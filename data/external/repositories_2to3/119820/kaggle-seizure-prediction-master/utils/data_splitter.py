...
def split_data_with_overlap(data_grouped_by_hour, valid_size, overlap_size, window_size,
    ...
    interictal_hours_indexes = list(range(len(data_grouped_by_hour['interictal'])))
    preictal_hours_indexes = list(range(len(data_grouped_by_hour['preictal'])))
    ...
    def fill_data_list(class_label, indexes):
        ...
        for idx in indexes:
            ...
            if overlap:
                ...
                for i in range(divmod(data.shape[-1] - overlap_size, window_size - overlap_size)[0]):
    ...
    x_valid[:], y_valid[:] = list(zip(*combined))
