...
class CuisinierTFIDF(Cuisinier):
    ...
    def calculateTFIDF(self):
        ...
        for cuisine in list(self.cuisineMatrix.keys()):
            maxFreq = max(self.cuisineMatrix[cuisine].values())
            for ingredient, iFreq in list(self.cuisineMatrix[cuisine].items()):
    ...
    def classify(self, recipe):
        ...
        for ingredient in recipe.ingredients:
            if ingredient in self.tfidfScores:
                for cuisine, score in list(self.tfidfScores[ingredient].items()):
        ...
        bestCuisine = max(list(cuisineScores.keys()),
        ...
        while bestScore == contenderScore and len(cuisineScores) > 0:
            contender = max(list(cuisineScores.keys()),
                            key=(lambda key: cuisineScores[key]))
