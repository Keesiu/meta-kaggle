...
def get_settings(settings_file):
    ...
    if 'CVITERCOUNT' not in list(settings.keys()):
        settings.update({'CVITERCOUNT': 10})

    # add missing default and update file paths settings to have full absolute paths
    for settings_field in list(default_directories.keys()):
        if settings_field not in list(settings.keys()):
    ...
    if 'CLASSIFIER' in list(settings.keys()):
        try:
    ...
    if 'CLASSIFIER_OPTS' not in list(settings.keys()):
...
class DataAssembler:
    ...
    def _parse_segment_names(self):
        ...
        * segment dictionaries {subject:{ictyp:[segment names, ...]}}
        ...
        # then enforce tuple
        for subject in list(segments.keys()):
            for ictyp in list(segments[subject].keys()):
...
class Sequence_CV:
    def __init__(self, segments, metadata, r_seed=None, n_iter=10):
        ...
        self.segments = list([segment.split(".")[0] for segment in segments])
...
def output_csv(prediction_dict, settings, verbose=False):
    ...
    with open(output_file_path, 'w') as output_fh:
        ...
        for segment in list(prediction_dict.keys()):
...
def build_model_pipe(settings):
    ...
    if 'BAGGING' in list(settings.keys()):
...
def combined_auc_score(settings, auc_scores, subj_pred=None):
    ...
    if 'RFE' in settings:
        ...
        predictions, labels, weights, segments = list(map(np.hstack,
                                         list(zip(*list(subj_pred.values())))))
