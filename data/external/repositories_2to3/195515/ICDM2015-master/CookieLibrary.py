...
def loadCookies(cookiefile,DictHandle,DictCookie,DictComputerOs,DictComputerV,DictCountry,DictAnnC1,DictAnnC2):
    ...
    maxindex=np.int(np.max(list(DictCookie.values())))
...
def fullCandidates(device,XDevices,XCookies,IPDev,IPCoo,DeviceIPS,DictHandle):
    ...
    ips=list(DeviceIPS[device].keys())
    ...
    for k,v in Candidates.items():
        ...
        for coo in v:
            ...
            for odev in OtherDevices:
                devp=devp | set(DevProps.get(odev,list(dict().keys())))
                devi=devi | set(DeviceIPS.get(odev,list(dict().keys())))
...
def bestSelection(predictions, OriginalIndex, values,Groups):
    ...
    for k,v in OriginalIndex.items():
        ...
        Orden=sorted(list(range(len(scores))),key=lambda x:-scores[x])
...
def FullTraining(YTR,XTR,XTST,OriginalIndexTR,OriginalIndexTST,DevicesTrain,Groups,Labels):
    ...
    skf = sklearn.cross_validation.KFold(len(list(OriginalIndexTR.keys())),n_folds=NFOLDS,random_state=0)
    ...
    for (train,test) in skf:
        ...
        for i in train:
             ...
             trainind.extend(list(OriginalIndexTR[devtr].values()))

        for i in test:
            devtr=DevicesTrain[i,1]
            testdev.append(devtr)
            testind.extend(list(OriginalIndexTR[devtr].values()))
