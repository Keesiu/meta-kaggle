...
def view_ride_speed(ride):
  ...
  acc = acc[1:-1]
  plt.plot(list(range(len(distances))), distances, 'b-')
  plt.plot(list(range(len(smoothed))), smoothed, 'r-')
  plt.plot(list(range(len(acc))), acc, 'g-')
  plt.plot(list(range(len(distances))), [0] * len(distances), 'm-')
  plt.show()
...
def get_ride_histograms(distances, normalized=False, version=1):
  ...
  if version == 1:
    hists = [
        np.histogram(numbers1, bins=list(range(0, 50, 4)))[0],
        np.histogram(numbers1[numbers1 < 20], bins=list(range(0, 20, 2)))[0],
        np.histogram(numbers2[-(numbers2>-4) -(numbers2<3)], bins=[-4 + i * 0.7 for i in range(10)])[0],
    ]
  else:
    hists = [
        np.histogram(numbers1, bins=list(range(0, 40, 4)))[0],
        np.histogram(numbers1, bins=list(range(0, 20, 1)))[0],
...
def build_features_acc(ride, version=1):
  ...
  if version == 1:
    sta_hist = np.histogram(speed_times_acc, bins=list(range(-400, 400, 40)))[0]
  else:
    sta_hist = np.histogram(speed_times_acc, bins=list(range(-500, 500, 20)))[0]
  if version == 1:
...
def build_features(ride, normalized=False, version=1):
  ...
  if version == 1:
    h_g_forces = np.histogram(g_forces, bins=list(range(0, 600, 50)))[0]
  else:
    h_g_forces = np.histogram(g_forces, bins=list(range(0, 600, 10)))[0]
...
def build_features_big(ride_orig):
  ...
  hists = [
      np.histogram(distances, bins=list(range(0, 50, 4)))[0] / (len(distances) + 1.0),
      np.histogram(distances[distances < 20], bins=list(range(0, 20, 2)))[0],
      np.histogram(acc, bins=[-4 + i * 0.7 for i in range(10)])[0] / (len(acc) + 1.0),
      np.histogram(g_forces, bins=list(range(0, 600, 10)))[0],
      np.histogram(acc * distances2, bins=list(range(-500, 500, 20)))[0],
...
def build_features_big_v2(ride_orig):
  ...
  hists = [
      np.histogram(distances, bins=list(range(0, 50, 3)))[0] / (len(distances) + 1.0),
      np.histogram(distances[distances < 20], bins=list(range(0, 20, 1)))[0],
      ...
      np.histogram(g_forces, bins=list(range(0, 600, 10)))[0],
      np.histogram(acc * distances2, bins=list(range(-500, 500, 20)))[0],
      np.histogram(acc_orig * distances2, bins=list(range(-500, 500, 20)))[0],
...
def get_accelerations_v2(ride):
  distances = euclidian_distances(ride)
  accelerations = [distances[i] - distances[i-1] for i in range(1, len(distances))]
  bucketed = np.digitize(accelerations, np.array(list(range(-30, 30, 3))) / 10.0)
  words = ['a%s_%s' % (bucketed[i-1], bucketed[i]) for i in range(1, len(bucketed))]
  return words
...
def get_similarities(rides):
  ...
  def get_words(ride):
    ...
    # bucket the values
    lengths = np.digitize(lengths * 0.3, list(range(0, 200 * GAP, 1 * GAP)))
    #times = [int(t) for t in times]
    angles = np.digitize(angles, list(range(0, 180, 10)))
