...
def slice_location_finder(slicepath2metadata):
    ...
    for sp, metadata in slicepath2metadata.items():
        if 'sax' in sp:
    ...
    if len(slicepath2midpix) <= 1:
        for sp, midpix in slicepath2midpix.items():
            slicepath2position[sp] = 0.
    else:
        # find the keys of the 2 points furthest away from each other
        max_dist = -1.0
        max_dist_keys = []
        for sp1, midpix1 in slicepath2midpix.items():
            for sp2, midpix2 in slicepath2midpix.items():
                if sp1 == sp2:
                    continue
