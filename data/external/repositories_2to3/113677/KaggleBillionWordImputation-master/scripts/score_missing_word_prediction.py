...
if __name__ == "__main__":
    ...
    if len(golden) != len(pred):
        print("WARNING: %d golden positions != %d predicted positions" \
            % (len(golden), len(pred)), file=sys.stderr)
    ...
    if args.guessed_words or args.real_words:
        ...
        print("Annotating sentences", file=sys.stderr)
        for i, (sentence, guessed, actual) in enumerate(zip(sys.stdin, args.guessed_words, args.real_words)):
            if golden[i] != pred[i]: # errors
                sentence = sentence.rstrip()
                words = tokenize_words(sentence)
                
                if args.model: # add probability annotations
                    for j, (w, (p, _, _)) in enumerate(zip(words, model.full_scores(sentence))):
                        words[j] = '%s{%.2f}' % (w, p)
