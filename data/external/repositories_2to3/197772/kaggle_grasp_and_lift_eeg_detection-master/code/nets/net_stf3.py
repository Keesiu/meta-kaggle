...
def create_net(train_source, test_source, batch_size=32, max_epochs=100, rate=None, patience=20): 
        
    if rate is None:
        # Subject 9 was more subject to blowing up for some reason on this net
        rate = 0.03 if (train_source and train_source.subject == 9) else 0.04
    ...
    layers = [
        ...
        #
        LF(Conv1DLayer, num_filters=FEATURES, filter_size=9, nonlinearity=None),
        ...
        LF(ReshapeLayer, shape=([0], -1, 8*FEATURES)),
        LF(DimshuffleLayer, pattern=(0,2,1)),        
        LF(NINLayer, num_units=FEATURES, nonlinearity=None),  
        #
        LF(Conv1DLayer, num_filters=16, filter_size=7, pad="same", 
            nonlinearity=very_leaky_rectify),
        LF(MaxPool1DLayer, pool_size=3, stride=2, ignore_border=False),
        #
        LF(Conv1DLayer, num_filters=32, filter_size=7, pad="same",
            nonlinearity=very_leaky_rectify, untie_biases=True),
        LF(MaxPool1DLayer, pool_size=3, stride=2, ignore_border=False),
        #
        LF(Conv1DLayer, num_filters=64, filter_size=7, pad="same",
...
# Score 0.9809 on local validation. Needed to train subject 9 at rate = 0.03 though. (net_stf3b_e30)
# LB score 0.96816
