...
def read_target(set):
    ...
    df = df.rename(columns = dict(list(zip(df.columns, ["Target", "Details"]))))
    ...
    original_index = np.array(list(zip(df.index, df.index))).flatten()
    df = pd.concat([df, df_inverse])
    df.index = list(range(0,len(df),2))+list(range(1,len(df),2))
...
def read_data(set):
    try:
        ...
        original_index = np.array(list(zip(features.index, features.index))).flatten()
        features = pd.concat([features, features_inverse])
        features.index = list(range(0,len(features),2))+list(range(1,len(features),2))
