...
def get_stats_percent(cuisine_type_count):
    ...
    cuisine_type_count_percent = {w: round(c/size,3) for (w,c) in list(cuisine_type_count.items()) }
    cuisine_type_count_percent = sorted(list(cuisine_type_count.items()), key=lambda x: x[1], reverse=1)
...
if __name__ == '__main__':
    ...
    top10 = sorted(list(cuisine_type_bag['irish'].items()), key=lambda x: x[1], reverse=1)
    print(top10[:10])
    print("")
    print("italian ingredients bow counts")
    ##print cuisine_type_bag['irish']
    top10 = sorted(list(cuisine_type_bag['italian'].items()), key=lambda x: x[1], reverse=1)
    print(top10[:10])
