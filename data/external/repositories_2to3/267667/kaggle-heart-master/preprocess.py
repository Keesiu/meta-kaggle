...
def sample_augmentation_parameters():
    ...
    res = dict()
    try:
        sample = next(quasi_random_generator)
    except ValueError:
        quasi_random_generator = quasi_random.scrambled_halton_sequence_generator(dimension=len(augmentation_params),
                                                                                  permutation='Braaten-Weller')
        sample = next(quasi_random_generator)

    for rand, (key, (a, b)) in zip(sample, iter(augmentation_params.items())):
...
def put_in_the_middle(target_tensor, data_tensor, pad_better=False, is_padded=None):
    ...
    target_indices, data_indices = list(zip(*t_sh))
    ...
    if pad_better:
        if target_indices[0].start:
            for i in range(0, target_indices[0].start):
                target_tensor[i] = data_tensor[0]
        if target_indices[0].stop:
            for i in range(target_indices[0].stop, len(target_tensor)):
...
def slice_location_finder(metadata_dict):
    ...
    if len(datadict)<=1:
        ...
        max_dist = -1.0
        max_dist_keys = []
        for key1, data1 in datadict.items():
            for key2, data2 in datadict.items():
                if key1==key2:
                    continue
    ...
    sorted_indices = [key for key in sorted(iter(datadict.keys()), key=lambda x: datadict[x]["relative_position"])]
