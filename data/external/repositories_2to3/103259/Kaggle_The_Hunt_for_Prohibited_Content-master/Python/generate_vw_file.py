...
eng_rusTranslateTable = dict(list(zip(engChars, rusChars)))
rus_engTranslateTable = dict(list(zip(rusChars, engChars)))
    ...
    with open(tsvFile, "rb") as tsvReader:
        ...
        for i, item in enumerate(itemReader):
            ...
            if (i+1)%10000 == 0:
                ...
                print(( list(catCounts.values())[:5] ))
                print( "Counts for subcategory:" )
                print(( list(subCatCounts.values())[:5] ))
                print( "Counts for keys of attrs:" )
                print(( list(attrsKeyCounts.values())[:5] ))
                print( "Counts for values of attrs:" )
                print(( list(attrsValCounts.values())[:5] ))
                print( "Counts for words in title and description:" )
                print(( list(wordCounts.values())[:5] ))
                    
    # get the index
    for index, (cat, count) in enumerate(catCounts.items()):
        catIndex[cat] = index+1

    for index, (subCat, count) in enumerate(subCatCounts.items()):
        subCatIndex[subCat] = index+1

    for index, (attrsKey, count) in enumerate(attrsKeyCounts.items()):
        attrsKeyIndex[attrsKey] = index+1

    for index, (attrsVal, count) in enumerate(attrsValCounts.items()):
        attrsValIndex[attrsVal] = index+1

    for index, (word, count) in enumerate(wordCounts.items()):
