...
def visualize_misclassifications(data, labels_true, names, probs, classnames, outdir, N=4):
    ...
    for i, label_true, prob in zip(list(range(num_samples)), labels_true, probs):
        ...
        rand_indices_true = np.random.choice(list(range(examples_true.shape[0])), size=(2 * N - 1), replace=replace_true)
        rand_indices_false = np.random.choice(list(range(examples_false.shape[0])), size=(2 * N), replace=replace_false)
