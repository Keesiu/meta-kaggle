...
def predict_model(expid, mfile=None):
    ...
    for e, test_data in zip(itertools.count(start=1), buffering.buffered_gen_threaded(create_test_gen())):
        ...
        for b in range(num_batches_chunk):
            ...
            network_outputs_dict = {list(output_layers.keys())[i]: network_outputs[i] for i in range(len(output_layers))}
