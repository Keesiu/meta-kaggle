...
idx = T.lscalar('idx')
givens_train = {}
for l_in, x in zip(model.l_ins, xs_shared):
    givens_train[l_in.input_var] = x[idx * config().batch_size:(idx + 1) * config().batch_size]
for l_target, y in zip(model.l_targets, ys_shared):
...
if config().restart_from_save:
    ...
    chunk_idxs = list(range(start_chunk_idx, config().max_nchunks))
    ...
    chunk_idxs = list(range(config().max_nchunks))
    losses_train = []
    losses_eval_valid = []
    crps_eval_valid = []
    start_chunk_idx = 0
...
for chunk_idx, (xs_chunk, ys_chunk, patient_idx) in zip(chunk_idxs,
