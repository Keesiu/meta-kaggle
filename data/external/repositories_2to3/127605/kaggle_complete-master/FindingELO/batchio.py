...
def data_parse(params,enginename = "stockfish",follow_uci=False):
    ...
    data["Results"] = list(map(clean_result,results))
    data["WhiteEloScore"] = whiteelos
    data["BlackEloScore"] = blackelos
    data["MoveScores"] = data[["MoveScores"]].apply(lambda r: [list(map(toint,x)) for x in r.str.split(" ")])
    ...
    data["WhiteAdvantageMoveScores"] = data["MoveScores"].apply(lambda x:[elem for elem in x if elem >= 0])
    data["BlackAdvantageMoveScores"] = data["MoveScores"].apply(lambda x:[elem for elem in x if elem < 0])
    #Setup partitions
