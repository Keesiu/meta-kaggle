...
def config(model_def, pretrained_model, gpu, image_dim, image_mean_file):
  ...
  IMAGE_DIM = image_dim
  CROPPED_DIM = list(NET.blobs.values())[0].width
  ...
  CROPPED_IMAGE_MEAN = IMAGE_MEAN[IMAGE_CENTER:IMAGE_CENTER + CROPPED_DIM,
                                  ...
                                  :]
  BATCH_SIZE = list(NET.blobs.values())[0].num  # network batch size
  NUM_OUTPUT = list(NET.blobs.values())[-1].channels  # number of output classes
