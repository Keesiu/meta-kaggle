...
class FeatureMaxPoolingLayer(object):
    ...
    def output(self, *args, **kwargs):
        ...
        if self.implementation == 'max_pool':
            ...
            shuffle_order = list(range(0, self.feature_dim)) + list(range(self.feature_dim + 1, input.ndim)) + [self.feature_dim]
            unshuffle_order = list(range(0, self.feature_dim)) + [input.ndim - 1] + list(range(self.feature_dim, input.ndim - 1))
