...
def plot_train_test(subject, data_path):
    ...
    zz = z[np.where(np.array(markers) != ' ')[0], :]
    ax = plt.subplot(111)
...
def plot_sequences(subject, data_path, test_labels):
    # data train
    filenames_grouped_by_hour = pickle.load(open('filenames.pickle'))
    ...
    marker_type = {'D': 'diamond', 's': 'square', '^': 'triangle_up', 'd': 'thin_diamond', 'h': 'hexagon1',
                   '*': 'star', 'o': 'circle', '.': 'point', 'p': 'pentagon', 'H': 'hexagon2',
                   'v': 'triangle_down', '8': 'octagon', '<': 'triangle_left', '>': 'triangle_right'}
    marker_list = list(marker_type.keys()) * 50
