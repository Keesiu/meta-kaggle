...
def predict_test(feat_importance_fun, mappings, model, na_fill_value, predict, silent, staged_predict,
    ...
    if not silent:
        ...
        if feat_importance is not None:
            ...
            feat_importance.index = list(range(1, len(feat_importance) + 1))
...
def train_and_predict(train_data_file, test_data_file, target_col, test_pred_file,
    ...
    if 0 < bootstrap < 1.0:
        ...
        bootstrap_ix = random.sample(list(range(len(train_x))), bootstrap_len)
    ...
    predict = lambda fitted_model_pred_x18: fitted_model_pred_x18[0].predict(fitted_model_pred_x18[1])
    staged_predict = lambda fitted_model_pred_x19: [predict((fitted_model_pred_x19[0], fitted_model_pred_x19[1]))]
    ...
    if model_type == "RandomForestRegressor":
        ...
        predict = lambda fitted_model_pred_x2: continuous_predict(model=fitted_model_pred_x2[0], x=fitted_model_pred_x2[1])

    elif model_type == "RandomForestClassifier":
        if model is None:
            model = RandomForestClassifier(**fit_args)
            model.fit(X=train_x, y=train_y, **extra_fit_args)
        predict = lambda fitted_model_pred_x3: pred_proba(model=fitted_model_pred_x3[0], x=fitted_model_pred_x3[1])
        staged_predict = lambda fitted_model_pred_x4: [predict((fitted_model_pred_x4[0], fitted_model_pred_x4[1]))]

    elif model_type == "ExtraTreesRegressor":
        if model is None:
            model = ExtraTreesRegressor(**fit_args)
            model.fit(X=train_x, y=train_y, **extra_fit_args)
        predict = lambda fitted_model_pred_x5: continuous_predict(model=fitted_model_pred_x5[0], x=fitted_model_pred_x5[1])

    elif model_type == "ExtraTreesClassifier":
        if model is None:
            model = ExtraTreesClassifier(**fit_args)
            model.fit(X=train_x, y=train_y, **extra_fit_args)
        predict = lambda fitted_model_pred_x6: pred_proba(model=fitted_model_pred_x6[0], x=fitted_model_pred_x6[1])
        staged_predict = lambda fitted_model_pred_x7: [predict((fitted_model_pred_x7[0], fitted_model_pred_x7[1]))]
        ...
        predict = lambda fitted_model_pred_x8: continuous_predict(model=fitted_model_pred_x8[0], x=fitted_model_pred_x8[1])
        staged_predict = lambda fitted_model_pred_x9: staged_pred_continuous(model=fitted_model_pred_x9[0], x=fitted_model_pred_x9[1])
        if load_type == "pred_at" and fit_args['n_estimators'] < model.n_estimators:
            if not silent:
                print(("Predict using %d trees" % fit_args['n_estimators']))
            predict = lambda fitted_model_pred_x: staged_pred_continuous_at_n(model=fitted_model_pred_x[0], x=fitted_model_pred_x[1],
        ...
        if model is None:
            ...
            model.n_estimators += fit_args['n_estimators']
            model.fit(X=train_x, y=train_y)
        staged_predict = lambda fitted_model_pred_x10: staged_pred_proba(model=fitted_model_pred_x10[0], x=fitted_model_pred_x10[1])
        predict = lambda fitted_model_pred_x11: pred_proba(model=fitted_model_pred_x11[0], x=fitted_model_pred_x11[1])
        if load_type == "pred_at" and fit_args['n_estimators'] < model.n_estimators:
            if not silent:
                print(("Predict using %d trees" % fit_args['n_estimators']))
            predict = lambda fitted_model_pred_x1: staged_pred_proba_at_n(model=fitted_model_pred_x1[0], x=fitted_model_pred_x1[1],
                                                                            n=fit_args['n_estimators'])
    elif model_type == "LogisticRegression":
        if model is None:
            model = LogisticRegression(**fit_args)
            model.fit(X=train_x, y=train_y)
        predict = lambda fitted_model_pred_x12: pred_proba(model=fitted_model_pred_x12[0], x=fitted_model_pred_x12[1])
        staged_predict = lambda fitted_model_pred_x13: [predict((fitted_model_pred_x13[0], fitted_model_pred_x13[1]))]
        ...
        if model is None:
            ...
            model.fit(X=train_x, y=train_y)
        predict = lambda fitted_model_pred_x14: pred_proba(model=fitted_model_pred_x14[0], x=fitted_model_pred_x14[1])
        staged_predict = lambda fitted_model_pred_x15: [predict((fitted_model_pred_x15[0], fitted_model_pred_x15[1]))]
        feat_importance_fun = lambda fitted_model: None
        ...
        if model is None:
            ...
            model.fit(X=train_x, y=train_y)
        predict = lambda fitted_model_pred_x16: pred_proba(model=fitted_model_pred_x16[0], x=fitted_model_pred_x16[1])
        staged_predict = lambda fitted_model_pred_x17: [predict((fitted_model_pred_x17[0], fitted_model_pred_x17[1]))]
        feat_importance_fun = lambda fitted_model: None
