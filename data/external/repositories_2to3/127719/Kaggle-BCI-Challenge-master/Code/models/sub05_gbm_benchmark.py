...
train = pd.DataFrame(columns=['subject','session','feedback_num','start_pos'] + ['Cz_' + s for s in map(str,list(range(261)))],index=list(range(5440)))
counter = 0
...
for i in train_subs:
    for j in range(1,6):
        ...
        for k in fb.index:
                ...
                temp2.index = ['Cz_' + s for s in map(str,list(range(261)))]
                train.loc[counter,['Cz_' + s for s in map(str,list(range(261)))]] = temp2
...
test = pd.DataFrame(columns=['subject','session','feedback_num','start_pos'] + ['Cz_' + s for s in map(str,list(range(261)))],index=list(range(3400)))
...
for i in test_subs:
    for j in range(1,6):
        ...
        for k in fb.index:
                temp2 = temp.loc[int(k):int(k)+260,'Cz']
                temp2.index = ['Cz_' + s for s in map(str,list(range(261)))]
                test.loc[counter,['Cz_' + s for s in map(str,list(range(261)))]] = temp2
