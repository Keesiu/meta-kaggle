...
def summary_svm(cmd="ls ../data/submission_svm_*.json"):
    ...
    json_list.sort(key=lambda json_file_json_dict:
                   (json_file_json_dict[1]["param_dict"]["scaler"],
                    json_file_json_dict[1]["param_dict"]["kernel"],
                    json_file_json_dict[1]["param_dict"]["class_weight"]))
...
def summary_gb(cmd="ls ../data/submission_gb_*.json"):
    ...
    for json_file in file_list:
        with open(json_file, 'r') as fp:
            json_list.append((json_file, json.loads(fp.read())))
    json_list.sort(key=lambda json_file_json_dict1:
                   (json_file_json_dict1[1]["param_dict"]["loss"],
                    json_file_json_dict1[1]["param_dict"]["max_depth"],
                    json_file_json_dict1[1]["param_dict"]["min_samples_leaf"]))
...
def summary_rf(cmd="ls ../data/submission_rf_*.json"):
    ...
    for json_file in file_list:
        with open(json_file, 'r') as fp:
            json_list.append((json_file, json.loads(fp.read())))
    json_list.sort(key=lambda json_file_json_dict2:
                   (json_file_json_dict2[1]["param_dict"]["criterion"],
                    json_file_json_dict2[1]["param_dict"]["min_samples_leaf"],
                    json_file_json_dict2[1]["param_dict"]["min_samples_split"]))
