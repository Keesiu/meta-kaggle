...
def bin_exactsamename(authors):
	bins = defaultdict(set)
 	for i, (id, a) in enumerate(authors.items()):
	...
	bins = defaultdict(set)
 	for i, (id, a) in enumerate(authors.items()):
 		if ':' not in a['name']:
	 		...
	 		ngrams = list(zip(*[tokens[j:] for j in range(nw)]))
			for p in ngrams:
				pg = ' '.join(p)
				if len(pg) > len(p)*2-1:
					bins[pg].add(id)
	...
	bins = defaultdict(set)
 	for i, (id, a) in enumerate(authors.items()):
 		if ':' not in a['fullname']:
 			lname = a['fullname']
	 		ngrams = list(zip(*[lname[j:] for j in range(n)]))
			for p in ngrams:
...
def main():
	...
	bins = sorted([(len(bv), blabel, bv) for blabel, bv in bins.items()], reverse=True)
